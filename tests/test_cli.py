...
class DotCliTests(unittest.TestCase):
    ...
    def test_kubuntu_desktop_manages_private_native_krdp(self):
        ...
        display = (ROOT / "scripts/krdp-display").read_text()
        ...
        self.assertIn("krdp-display", panel_watch)
        self.assertIn("MIN_STREAM_BYTES = 65536", panel_watch)
        self.assertIn('re.findall(r"bytes_sent:(\\d+)"', panel_watch)
        self.assertIn("elif streaming():", panel_watch)
        self.assertIn('TARGET_WIDTH = 2560', display)
        self.assertIn('TARGET_HEIGHT = 1600', display)
        self.assertIn('TARGET_SCALE = 1.55', display)
        self.assertIn('def target_mode(', display)
        self.assertIn('["kscreen-doctor", "-j"]', display)
        self.assertIn('STATE_FILE.unlink(missing_ok=True)', display)
        self.assertIn("krdp-display", task)
