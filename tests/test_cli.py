...
class DotCliTests(unittest.TestCase):
    ...
    def test_kubuntu_meshcentral_integration_is_explicit_and_documented(self):
        ...
        self.assertIn(
            'getpass.getpass("MeshCentral Linux install command: ")',
            cli,
        )
        self.assertIn('if action == "enroll-and-store":', cli)
        self.assertIn(
            'choices=["status", "enroll", "enroll-stored", "enroll-and-store"]',
            cli,
        )
        self.assertIn('if action in {"enroll", "enroll-and-store"}:', cli)
        self.assertIn("install_meshcentral_enrollment(enrollment)", cli)
        self.assertIn('getattr(args, "install_command_stdin", False)', cli)
        self.assertIn('"--install-command-stdin"', cli)
    ...
    def test_nomachine_is_private_to_the_kubuntu_tailnet(self):
        ...
        readiness = (ROOT / "scripts/wait-nomachine-tailscale").read_text()
        ...
        self.assertIn('port: "4011:4999"', task)
        self.assertIn("state: enabled", task)
        self.assertIn("EnableLocalNetworkBroadcast", task)
        self.assertIn("EnableNetwork", task)
        self.assertIn("EnableUPnP", task)
        self.assertIn("UserScriptAfterSessionStart", task)
        self.assertIn("UserScriptAfterSessionDisconnect", task)
        self.assertIn("UserScriptAfterRemoteResize", task)
        self.assertIn("nomachine-display-connect", task)
        self.assertIn("nomachine-display-disconnect", task)
        self.assertIn("EnableScreenBlanking", task)
        self.assertIn("EnableScreenBlankingEffect", task)
        self.assertIn("EnableLockScreen", task)
        self.assertIn("gamescope", task)
        self.assertIn("nomachine-client", task)
        self.assertIn("REMOTE_WIDTH = 1920", display)
        ...
        self.assertIn("GAMESCOPE_SCALE = 1.5", client)
        ...
        self.assertIn("~/.local/bin/nomachine-client", desktop)
        self.assertIn("~/.local/bin/nomachine-client", session_desktop)
        self.assertNotIn("Exec=nomachine-client", desktop)
        self.assertIn("STATE_FILE.unlink(missing_ok=True)", display)
        self.assertIn('value: "0"', task)
        self.assertIn("local keyboard and mouse remain active", docs)
        self.assertNotIn("REMOTE_BRIGHTNESS", display)
    ...
    def test_kubuntu_desktop_manages_private_native_krdp(self):
        ...
        scale = (ROOT / "scripts/krdp-scale").read_text()
        ...
        self.assertIn('flock -n 9', client)
        self.assertIn('krdp-client.lock', client)
        self.assertIn('pgrep -x xfreerdp', client)
        self.assertIn('/proc/$existing_pid/cmdline', client)
        self.assertIn('"/v:${host}:3389"', client)
        self.assertIn('Desktop (KRdp) is already connected', client)
        ...
        self.assertIn("krdp-audio", panel_watch)
        self.assertIn("reap_audio()", panel_watch)
        self.assertIn("scripts/krdp-audio", task)
        self.assertIn("pulseaudio-utils", task)
        self.assertIn("krdp-audio-client", client)
        self.assertIn('ROOT / "config/rdp/krdp-audio-client"', DOT.read_text())
        ...
        self.assertIn("Install the KRdp scale-only session manager", task)
        self.assertIn("Remove the retired KRdp physical-display manager", task)
        ...
        self.assertNotIn("/scale-desktop:", client)
    ...
    def test_native_plasma_panel_and_application_palette_are_managed(self):
        ...
        self.assertNotIn(
            "LOCKSCREEN_PACKAGE_SOURCE / \"contents/lockscreen\"",
            cli,
        )
    ...
    def test_kubuntu_manages_vscode_and_fullscreen_rdp_files(self):
        ...
        self.assertIn('candidate.get("primary")', remmina_launcher)
        self.assertIn('output["scale"]', remmina_launcher)
        self.assertIn('"--desktop-scale"', remmina_launcher)
        self.assertIn('"--device-scale"', remmina_launcher)
        self.assertIn('"resolution_width": resolution_width', remmina_launcher)
        self.assertIn('"resolution_height": resolution_height', remmina_launcher)
        ...
        self.assertIn('"rdp_desktopScaleFactor"', remmina_preferences)
        ...
        self.assertIn('"sound": "local"', remmina_launcher)
        self.assertIn('"microphone": "sys:pulse"', remmina_launcher)
        self.assertIn(
    ...
    def test_kubuntu_uses_separate_clipboard_and_emoji_tools(self):
        ...
        copyq_helper = (ROOT / "config/copyq/dot-copyq-paste").read_text()
        copyq_acceptance = (ROOT / "scripts/test-copyq-paste").read_text()
        copyq_theme = (ROOT / "config/copyq/github-dark.ini").read_text()
        copyq_tasks = (ROOT / "ansible/tasks/copyq.yml").read_text()
        uinput_rule = (ROOT / "config/udev/99-dotfiles-uinput.rules").read_text()
        ...
        self.assertIn("config('maxitems', 1100)", copyq)
        self.assertIn("config('clipboard_mime_size_limit', '.*:100M')", copyq)
        self.assertIn("config('autostart', false)", copyq)
        ...
        self.assertIn("ConditionEnvironment=WAYLAND_DISPLAY", copyq_service)
        self.assertIn("PartOf=graphical-session.target", copyq_service)
        self.assertIn("WantedBy=graphical-session.target", copyq_service)
        self.assertNotIn("WantedBy=default.target", copyq_service)
        ...
        self.assertIn('current_link="$app_root/current"', copyq_installer)
        self.assertIn('expected_file="$release_dir/image.sha512"', copyq_installer)
        self.assertIn("sha512sum --check --status", copyq_installer)
        self.assertIn("activate_release", copyq_installer)
        self.assertIn('mv -Tf -- "$temporary_link" "$current_link"', copyq_installer)
        self.assertIn('export APPDIR=', copyq_installer)
        self.assertIn(r'exec \"$current_run\"', copyq_installer)
        self.assertIn("global.dotfilesPasteVersion = 5", copyq)
        self.assertIn("global.dotfilesPasteHelper", copyq)
        self.assertIn("str(env('HOME')) + '/.local/bin'", copyq)
        self.assertNotIn("str(info('home')) + '/.local/bin'", copyq)
        self.assertIn("execute(global.dotfilesPasteHelper, '--paste')", copyq)
        self.assertNotIn("execute('dot-copyq-paste')", copyq)
        ...
        self.assertIn("global.dotfilesPasteHelper, '--check'", copyq_configurer)
        self.assertIn("/usr/bin/systemctl", copyq_helper)
        self.assertIn("/usr/bin/ydotool", copyq_helper)
        self.assertIn("--paste|--check", copyq_helper)
        self.assertIn("dot-uinput", copyq_tasks)
        self.assertIn("gpasswd", copyq_tasks)
        self.assertIn('KERNEL=="uinput", GROUP="dot-uinput", MODE="0660"', uinput_rule)
        self.assertIn("Meta+V", copyq_acceptance)
        self.assertIn("Ctrl+`", copyq_acceptance)
        self.assertIn("image/png", copyq_acceptance)
        self.assertIn('copyq("read", "?", "0"', copyq_acceptance)
        self.assertIn('copyq("disable")', copyq_acceptance)
        self.assertIn('"restart", "dot-copyq.service"', copyq_acceptance)
        ...
        self.assertIn("def copyq_runtime", cli)
        self.assertIn("def managed_clipboard_callback_ok", cli)
        self.assertIn("dot-clipboard-probe.service", cli)
        self.assertIn('elif action == "test":', cli)
        self.assertIn("dot-test-copyq-paste", cli)
        self.assertNotIn('["systemctl", "--user", "reenable", copyq_unit]', cli)
